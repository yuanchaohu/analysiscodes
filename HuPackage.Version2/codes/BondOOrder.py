    ...
    for m1 in range(-l, l + 1):
        for m2 in range(-l, l + 1):
            for m3 in range(-l, l + 1):
                if m1 + m2 + m3 ==0:
                    windex = wigner_3j(l, l, l, m1, m2, m3)
...
class BOO3D:
    ...
    def __init__(self, dumpfile, Neighborfile, faceareafile, *arg):
        self.dumpfile = dumpfile
        d = readdump(self.dumpfile)
        ...
        self.rhototal = self.ParticleNumber / (self.Boxlength[0] * self.Boxlength[1] * self.Boxlength[2])
    ...
    def qlmQlm(self, l, ppp, AreaR):
        ...
        ffacearea = open(self.faceareafile, 'r')
        smallqlm = []
        largeQlm = []
    ...
    def qlQl(self, l, ppp = [1,1,1], AreaR = 0, outputql = '', outputQl = '', results_path = '../../analysis/BOO'):
        ...
        if outputql:
            ...
            np.savetxt(results_path + outputql, smallql, fmt=numformat, header = names, comments = '')
        ...
        if outputQl:
            ...
            np.savetxt(results_path + outputQl, largeQl, fmt=numformat, header = names, comments = '')
        print ('-------------Calculate ql and Ql over-----------')
        return (smallql, largeQl)

    def sijsmallql(self, l, ppp = [1,1,1], AreaR = 0, c = 0.7, outputql = '', outputsij = '', results_path = '../../analysis/BOO'):
    ...
    def GllargeQ(self, l, ppp = [1,1,1], rdelta = 0.01, AreaR = 0, outputgl = 'GllargeQ.dat', results_path = '../../analysis/BOO'):
        ...
        np.savetxt(results_path + outputgl, results, fmt='%.6f', header = names, comments = '')
        print ('---------- Get Gl(r) results based on Ql over ---------')
        return results

    def Glsmallq(self, l, ppp = [1,1,1], rdelta = 0.01, AreaR = 0, outputgl = 'Glsmallq.dat', results_path = '../../analysis/BOO'):
        """ Calculate bond order spatial correlation function Gl(r) based on qlm
    ...
    def smallwcap(self, l, ppp = [1,1,1], AreaR = 0, outputw = '', outputwcap = '', results_path = '../../analysis/BOO'):
        ...
        if outputw:
            names = 'id  wl  l=' + str(l)
            numformat = '%d ' + '%.6f ' * (len(smallw[0]) - 1) 
            np.savetxt(results_path + outputw, smallw, fmt=numformat, header = names, comments = '')
        ...
        if outputwcap:
            ...
            numformat = '%d ' + '%.6f ' * (len(smallwcap[0]) - 1) 
            np.savetxt(results_path + outputwcap, smallwcap, fmt=numformat, header = names, comments = '')
        print ('------------- Calculate BOO w and normalized (cap) one over ----------------')
        return (smallw, smallwcap)


    def largeWcap(self, l, ppp = [1,1,1], AreaR = 0, outputW = '', outputWcap = '', results_path = '../../analysis/BOO'):
        """ Calculate wigner 3-j symbol boo based on Qlm
        ...
        if outputW:
            names = 'id  Wl  l=' + str(l)
            numformat = '%d ' + '%.6f ' * (len(largew[0]) - 1) 
            np.savetxt(results_path + outputW, largew, fmt=numformat, header = names, comments = '')
        ...
        if outputWcap:
            names = 'id  Wlcap  l=' + str(l)
            numformat = '%d ' + '%.6f ' * (len(largewcap[0]) - 1) 
            np.savetxt(results_path + outputWcap, largewcap, fmt=numformat, header = names, comments = '')
        print ('------------- Calculate BOO W and normalized (cap) one over ----------------')
        return (largew, largewcap)


    def timecorr(self, l, ppp = [1,1,1], AreaR = 0, dt = 0.002, outputfile = 'timecorr.dat', results_path = '../../analysis/BOO'):
