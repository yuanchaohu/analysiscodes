...
Docstr = """
         This module is responsible for calculating pair correlation functions in 2D and 3D
         The code is written refering to 'Allen book P183' covering unary to senary systems
         Use the code by 'from paircorrelationfunctions import gr3d/gr2d'
         gr3d/gr2d(inputfile).getresults(outputfile, rdelta, ppp, results_path)
...
class gr3d:
    """ Compute (Partial) pair correlation functions g(r) in three dimensional """
    ...
    def __init__(self, inputfile, *arg):
        ...
        self.Volume     = self.Boxlength[0] * self.Boxlength[1] * self.Boxlength[2]
    ...
    def getresults(self, outputfile, rdelta = 0.01, ppp = [1,1,1], results_path='../../analysis/gr/'):

        if len(self.Type) == 1:
            self.Unary(outputfile, rdelta, ppp, results_path)

        if len(self.Type) == 2: 
            self.Binary(outputfile, rdelta, ppp, results_path)

        if len(self.Type) == 3: 
            self.Ternary(outputfile, rdelta, ppp, results_path)

        if len(self.Type) == 4: 
            self.Quarternary(outputfile, rdelta, ppp, results_path)

        if len(self.Type) == 5: 
            self.Quinary(outputfile, rdelta, ppp, results_path)

        if len(self.Type) == 6: 
            self.Senary(outputfile, rdelta, ppp, results_path)


    def Unary(self, outputfile, rdelta = 0.01, ppp = [1,1,1], results_path='../../analysis/gr/'):
        ...
        #flattened = [x for arr in distanceall for x in arr]  #change distanceall to a pure list
        #Countvalue, BinEdge = np.histogram(flattened, bins = MAXBIN, range = (0, MAXBIN * rdelta))
        ...
        grresults  = grresults * 2 / self.ParticleNumber / self.SnapshotNumber / Nideal
    ...
    def Binary(self, outputfile, rdelta = 0.01, ppp = [1,1,1], results_path='../../analysis/gr/'):
        ...
        binleft  = BinEdge[:-1]   #real value of each bin edge, not index
        binright = BinEdge[1:]   #len(Countvalue) = len(BinEdge) - 1
        Nideal   = 4/3.0 * np.pi * (binright**3 - binleft**3)  
    ...
    def Ternary(self, outputfile, rdelta = 0.01, ppp = [1,1,1], results_path='../../analysis/gr/'):
    ...
    def Quarternary(self, outputfile, rdelta = 0.01, ppp = [1,1,1], results_path='../../analysis/gr/'):
    ...
    def Quinary(self, outputfile, rdelta = 0.01, ppp = [1,1,1], results_path='../../analysis/gr/'):
    ...
    def Senary(self, outputfile, rdelta = 0.01, ppp = [1,1,1], results_path='../../analysis/gr/'):
...
class gr2d:
    """ Compute (Partial) pair correlation functions g(r) in two dimension """
    ...
    def getresults(self, outputfile, rdelta = 0.01, ppp = [1,1], results_path='../../analysis/gr/'):
    ...
    def Unary(self, outputfile, rdelta = 0.01, ppp = [1,1], results_path='../../analysis/gr/'):
        print ('--------- This is a Unary System ------')
        ...
        for n in range(self.SnapshotNumber):
            for i in range(self.ParticleNumber - 1):
                RIJ = self.Positions[n][i+1:, :2] - self.Positions[n][i, :2]
                periodic = np.where(np.abs(RIJ / self.Boxlength) > 0.50, np.sign(RIJ), 0).astype(np.int)
                RIJ -= self.Boxlength * periodic * ppp    #remove PBC
                #RIJ -= np.dot(np.round(RIJ / self.Boxlength[np.newaxis, :]), np.diag(self.Boxlength)) * ppp #another way to remove PBC
                distance = np.sqrt(np.square(RIJ).sum(axis = 1))
    ...
    def Binary(self, outputfile, rdelta = 0.01, ppp = [1,1], results_path='../../analysis/gr/'):
        print ('--------- This is a Binary System ------')
        ...
        binleft  = BinEdge[:-1]   #real value of each bin edge, not index
        binright = BinEdge[1:]   #len(Countvalue) = len(BinEdge) - 1
        Nideal   = np.pi * (binright**2 - binleft**2) 
        grresults[:, 0]  = grresults[:, 0] * 2 / self.SnapshotNumber/ self.ParticleNumber / (Nideal * self.rhototal)
        grresults[:, 1]  = grresults[:, 1] * 2 / self.SnapshotNumber/ self.TypeNumber[0] / (Nideal * self.rhotype[0])
        grresults[:, 2]  = grresults[:, 2] * 2 / self.SnapshotNumber/ Nideal * self.Volume / self.TypeNumber[0]/ self.TypeNumber[1]/ 2.0
        grresults[:, 3]  = grresults[:, 3] * 2 / self.SnapshotNumber/ self.TypeNumber[1] / (Nideal * self.rhotype[1])
    ...
    def Ternary(self, outputfile, rdelta = 0.01, ppp = [1,1], results_path='../../analysis/gr/'):
        print ('--------- This is a Ternary System ------')
    ...
    def Quarternary(self, outputfile, rdelta = 0.01, ppp = [1,1], results_path='../../analysis/gr/'):
        print ('--------- This is a Quarternary System ------')
    ...
    def Quinary(self, outputfile, rdelta = 0.01, ppp = [1,1], results_path='../../analysis/gr/'):
        print ('--------- This is a Quinary System ------')
    ...
    def Senary(self, outputfile, rdelta = 0.01, ppp = [1,1], results_path='../../analysis/gr/'):
        """ Only calculate the overall g(r) at this stage """
        print ('--------- This is a Senary System ------')
