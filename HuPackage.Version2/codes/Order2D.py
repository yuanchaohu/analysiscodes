...
Docstr = """
         This module calculates bond orientational order in 2D
         """
...
class BOO2D:
    ...
    def __init__(self, dumpfile, Neighborfile, *arg):
        ...
        self.rhototal = self.ParticleNumber / (self.Boxlength[0] * self.Boxlength[1])
    ...
    def lthorder(self, l = 6, ppp = [1, 1]):
        ...
        for n in range(self.SnapshotNumber):
            ...
            for i in range(self.ParticleNumber):
                ...
                periodic = np.where(np.abs(RIJ / self.Boxlength) > 0.5, np.sign(RIJ), 0).astype(np.int)
    ...
    def tavephi(self, outputphi, outputavephi, avet, l = 6, ppp = [1, 1], dt = 0.002, results_path = '../../analysis/order2d/'):
        ...
        if outputphi:
            ...
            np.savetxt(results_path + outputphi, ParticlePhi, fmt= numformat, header = names, comments = '')
        ...
        if outputavephi:
            ...
            np.savetxt(results_path + outputavephi, averesults, fmt = numformat, header = names, comments = '')

        print ('---------Compute Particle Level PHI Over------------')

    def spatialcorr(self, outputfile, l = 6, ppp = [1, 1], rdelta = 0.01, results_path = '../../analysis/order2d/'):
    ...
    def timecorr(self, outputfile, l = 6, ppp = [1, 1], dt = 0.002, results_path = '../../analysis/order2d/'):
